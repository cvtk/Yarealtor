...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      inArr(arr, fltr) {
        if ( typeof(arr) === 'undefined' || !arr.length ) return true;
        if ( typeof(fltr) === 'undefined' || !fltr.length ) return true;

        return arr.some( el => fltr.some( fl => fl.value === el.value ) );
      },
      inRange(a, from, to) {
      ...
      iterateFields(obj) {
        return this.isEqual( obj.op.value, this.general.op.value ) &&
                ...
                this.inRange( obj.area_to, parseInt(this.object.area_from), parseInt(this.object.area_to) ) &&
                this.inRange( obj.cottage_area_from, parseInt(this.object.cottage_area_from), parseInt(this.object.cottage_area_to) ) &&
                this.inRange( obj.cottage_area_to, parseInt(this.object.cottage_area_from), parseInt(this.object.cottage_area_to) ) &&
                this.inArr( obj.rooms, this.object.rooms ) &&
                this.inArr( obj.type, this.object.type )
      },
