<template>
  <div :class="$style.street">
    ...
    <img :class="$style.street__loader" src="./assets/spin.svg" v-if="!dataReady">
    <div :class="$style.street__autocomplete_menu" v-show="showMenu === true">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onInput(event) {
        if ( !!this.local.trim() && !!this.parentId ) {
